...
class test_solver(solvertest.SolverTest):
    ...
    simple_puzzle = os.path.join(solvertest.SolverTest.data_dir, 
                                 'puzzle-01-simple')
    exported_puzzle = os.path.join(solvertest.SolverTest.tmp_dir, 
                                   'exported_puzzle')
    simple_puzzle_as_string = os.path.join(solvertest.SolverTest.data_dir, 
                                           'puzzle-01-simple-string')
    ...
    def test_getCurrentPuzzleString_gives_right_string(self):
        self._solver.load(self.simple_puzzle)
        puzzle_string = self._solver.getCurrentPuzzleString()

        file = open(self.simple_puzzle_as_string)
        correct_puzzle_string = file.read().strip()
        file.close()
